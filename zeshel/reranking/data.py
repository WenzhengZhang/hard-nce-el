...
class FullDataset(BasicDataset):
    ...
    def __getitem__(self, index):
        ...
        mention_marks = torch.zeros((self.max_num_candidates, self.max_len))
        ...
        for i, candidate_document_id in enumerate(candidate_document_ids):
            ...
            for j in range(mention_start, mention_end + 1):
                mention_marks[i][j] = 1

            input_lens[i] = len(mention_window) + len(candidate_prefix) + 3

        return encoded_pairs, type_marks, mention_marks, input_lens
...
class Logger(object):
    ...
    def log_zeshel_data_stats(self, data):
        ...
        def count_domains(dict_documents, report_lens=False):
        ...
        domain_count_all, lens = count_domains(documents, report_lens=True)
