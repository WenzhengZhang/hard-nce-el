...
from bert_model_embed import BertModel as CustomBertModel
from data import FullDataset, get_loaders, \
    load_zeshel_data, Logger, UnifiedDataset
from datetime import datetime
from model import Zeshel, UnifiedRetriever
...
def load_model(model_path, device, eval_mode=False, dp=False, custombert=False,
               memb2=False, inputmark=False, type_model='full'):
    package = torch.load(model_path) if device.type == 'cuda' else \
        torch.load(model_path, map_location=torch.device('cpu'))

    if custombert:
        encoder = CustomBertModel.from_pretrained('bert-base-uncased')
    else:
    ...
    if type_model == 'full':
        model = Zeshel(encoder, args.init, device, memb2, inputmark).to(device)
    else:
...
def main(args):
    ...
    if args.type_model == 'full':
        model = Zeshel(encoder, args.init, device, args.memb2, args.inputmark)
    else:
        if args.type_model == 'poly':
            attention_type = 'soft_attention'
        else:
    ...
    model = load_model(args.model, device, eval_mode=True, dp=dp,
                       custombert=args.custombert, memb2=args.memb2,
                       inputmark=args.inputmark, type_model=args.type_model)
    if args.eval_method == 'micro':
...
if __name__ == '__main__':
    ...
    parser.add_argument('--custombert', action='store_true',
                        help='use custom BERT?')
    parser.add_argument('--memb2', action='store_true',
                        help='use mention lookup embeddings 2-way?')
